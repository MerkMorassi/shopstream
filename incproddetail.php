...
mysql_free_result($result2);
$sSQL = 'SELECT pId,pSKU,'.getlangid('pName',1).','.getlangid('pDescription',2).','.$WSP.'pPrice,pSection,pListPrice,pSell,pStockByOpts,pStaticPage,pInStock,pBackOrder,pExemptions,'.(@$detailslink!='' ? "'' AS " : '').'pTax,pOrder,pDateAdded,'.(@$manufacturerfield!=''?'mfName,':'').getlangid('pLongDescription',4).' FROM products '.(@$manufacturerfield!=''?'LEFT OUTER JOIN manufacturer on products.pManufacturer=manufacturer.mfID ':'')."WHERE pDisplay<>0 AND pId='" . escape_string($prodid) . "'";
$result = mysql_query($sSQL) or print(mysql_error());
...
<?php
	if(! (@isset($showcategories) && @$showcategories==FALSE)){
		...
		print (@$usecsslayout ? '<div' : '<td colspan="3" align="left" valign="top"') . ' class="prodnavigation">' . $sstrong . (! @$usecsslayout ? '<p class="prodnavigation">' : '') . $tslist . (! @$usecsslayout ? '<br /><img src="images/clearpixel.gif" width="300" height="8" alt="" /></p>' . $estrong . '</td>' : '</div>');
...
<?php
		...
		if(! @$usecsslayout) print '<td width="70%" valign="top" class="detail">';
		if(is_array($prodoptions)){
			$rs['pPrice'] += $optdiff;
		}
		...
		if(@$noprice==TRUE){
			...
			if($currRate3!=0 && $currSymbol3!='') $extracurr .= str_replace('%s',number_format($rs['pPrice']*$currRate3,checkDPs($currSymbol3),$orcdecimals,$orcthousands),$currFormat3);
			if($extracurr!='') print '<div class="detailcurrency"><span class="extracurr" id="pricedivec' . $Count . '">' . ($rs['pPrice']==0 ? '' : $extracurr) . '</span></div>';
			if(! @$usecsslayout) print '<hr width="80%" class="currencyhr" />';
		}
		if(is_array($prodoptions)){
			if($optjs!='') print '<script language="javascript" type="text/javascript">/* <![CDATA[ */'.$optjs.'/* ]]> */</script>';
			if($prodoptions[0]['optType']==4) $thestyle=''; else $thestyle=' width="100%"';
			if($optionshtml!='') print '<div class="detailoptions"' . (! @$usecsslayout ? ' align="center"' : '') . '>' . (! @$usecsslayout ? '<table class="prodoptions detailoptions" border="0" cellspacing="1" cellpadding="1"' . $thestyle . '>' : '') . $optionshtml;
		...
		print (! @$usecsslayout ? '<p align="center">' : '');
		if(@$nobuyorcheckout == TRUE)
			print '&nbsp;';
		else{
			if($rs['pPrice']==0 && @$nosellzeroprice==TRUE){
		...
		if(! @$usecsslayout) print '</p><hr width="80%" />';
...
<?php	if(@$usecsslayout) print '<div class="reviewformblock">'; else print '<table border="0" cellspacing="0" cellpadding="2" width="100%" align="center">';
		if(@$usecsslayout) print '<div class="ectformline reviewformline">'; else print '<tr><td align="right">'?><div class="review reviewform reviewlabels"><span class="review reviewstar" style="color:#FF0000">*</span><?php print $xxRvRati?>:</div><?php if(!@$usecsslayout) print '</td><td>'?><div class="review reviewform reviewfields"><select size="1" name="ratingstars" class="review reviewform"><option value=""><?php print $xxPlsSel?></option><?php
			for($index=1; $index<=5; $index++){
				print '<option value="'.$index.'">'.$index.' '.$xxStars.'</option>';
			} ?></select></div><?php if(@$usecsslayout) print '</div>'; else print '</td></tr>';
		if(@$usecsslayout) print '<div class="ectformline reviewformline">'; else print '<tr><td align="right">'?><div class="review reviewform reviewlabels"><span class="review reviewstar" style="color:#FF0000">*</span><?php print $xxRvPosb?>:</div><?php if(!@$usecsslayout) print '</td><td>'?><div class="review reviewform reviewfields"><input type="text" size="20" name="reviewposter" maxlength="64" value="<?php print htmlspecials(@$_SESSION['clientUser'])?>" class="review reviewform" /></div><?php if(@$usecsslayout) print '</div>'; else print '</td></tr>';
		if(@$usecsslayout) print '<div class="ectformline reviewformline">'; else print '<tr><td align="right">'?><div class="review reviewform reviewlabels"><span class="review reviewstar" style="color:#FF0000">*</span><?php print $xxRvHead?>:</div><?php if(!@$usecsslayout) print '</td><td>'?><div class="review reviewform reviewfields"><input type="text" size="40" name="reviewheading" maxlength="253" class="review reviewform" /></div><?php if(@$usecsslayout) print '</div>'; else print '</td></tr>';
		if(@$usecsslayout) print '<div class="ectformline reviewformline">'; else print '<tr><td align="right">'?><div class="review reviewform reviewlabels"><?php print $xxRvComm?>:</div><?php if(!@$usecsslayout) print '</td><td>'?><div class="review reviewform reviewfields"><textarea name="reviewcomments" cols="38" rows="8" class="review reviewform"></textarea></div><?php if(@$usecsslayout) print '</div>'; else print '</td></tr>';
		if(@$usecsslayout) print '<div class="ectformline reviewformline">'; else print '<tr><td align="right">&nbsp;'?><?php if(!@$usecsslayout) print '</td><td>'?><div class="review reviewform reviewfields"><input type="submit" value="<?php print $xxSubmt?>" class="review reviewform reviewsubmit" /></div><?php if(@$usecsslayout) print '</div>'; else print '</td></tr>';
