...
if(@$_POST['updatestatus']=='1'){
	mysql_query("UPDATE orders SET ordTrackNum='" . escape_string(unstripslashes(@$_POST['ordTrackNum'])) . "',ordStatusInfo='" . escape_string(unstripslashes(@$_POST['ordStatusInfo'])) . "',ordPrivateStatus='" . escape_string(unstripslashes(@$_POST['ordPrivateStatus'])) . "',ordInvoice='" . escape_string(unstripslashes(@$_POST['ordInvoice'])) . "'" . (trim(@$_POST['shipcarrier'])!='' ? ',ordShipCarrier=' . trim(@$_POST['shipcarrier']) : '') . " WHERE ordID=" . @$_POST['orderid']) or print(mysql_error());
	updateorderstatus(@$_POST['orderid'], (int)trim(@$_POST['ordStatus']));
...
<?php
	...
	if($orderid=='new'){
		...
		$sSQL .= "ordPrivateStatus='" . escape_string(unstripslashes(@$_POST['ordPrivateStatus'])) . "',";
...
<?php		} ?>
					...
					<tr>
					  ...
					  <td align="left" colspan="3">
						...
						<td>&nbsp;&nbsp;<?php if(! $doedit) print '<input type="submit" value="' . $yyUpdate . '" ' . ($alldata['ordPayProvider']==20 ? 'onclick="updategooglestatus(\'message\',' . $_GET['id'] . ')" ' : '') . '/>'?></td>
						</tr></table>
					  </td>
					</tr>
...
?>
			...
			?></td>
			  <td valign="top"><?php
				...
				$result = mysql_query($sSQL) or print(mysql_error());
				if(mysql_num_rows($result) > 0 || ($rsOrders['cartGiftWrap']!=0 && $cartGiftMessage!='' && ! $isinvoice)){
					...
					if(mysql_num_rows($result) > 0) do{
						if($doedit){
							...
							if(is_null($rs2["optGroup"])){
								...
								if($rsl = mysql_fetch_assoc($result2)){
									if(abs($rsl['optType'])==1 || abs($rsl['optType'])==2 || abs($rsl['optType'])==4){
										...
										print htmlspecials(strip_tags($rs2['coCartOption'])) . '</textarea>';
									}
								}
								mysql_free_result($result2);
							}
							...
							print '<strong>' . $rs2["coOptGroup"] . ':</strong> ' . str_replace(array("\r\n","\n"),array('<br />','<br />'),htmlspecials(strip_tags($rs2['coCartOption']))) . '<br />';
						}
						if($doedit)
		...
		if(! $isprinter || $isinvoice){ ?>
			<tr class="cobll">
			  <td align="<?php print $tright?>" colspan="<?php print ($isinvoice?'5':'4')?>">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
				  <tr>
					<td align="center"><?php if($stockManage!=0 && $doedit) print ' '; else print '&nbsp;' ?><?php
			if($doedit){
				print '<table class="cobtbl" border="0" cellspacing="1" cellpadding="3"><tr>';
				if($stockManage!=0) print '<td class="cobll">&nbsp;<input type="checkbox" name="updatestock" value="ON" checked> <strong>' . $yyUpStLv . '</strong>&nbsp;</td>';
...
<?php
	...
	if(@$_POST['act']=='authorize'){
		...
		$maxitems=(int)($_POST['maxitems']);
		for($index=0; $index < $maxitems; $index++){
			updateorderstatus(trim(@$_POST['ordid' . $index]), (int)trim(@$_POST['ordStatus' . $index]));
		}
	}
	$hasfromdate=FALSE;
	$hastodate=FALSE;
		...
		<img src="adminimages/clearpixel.gif" width="10" height="2" alt="" /><br/>
			<table class="cobtbl" width="100%" border="0" cellspacing="1" cellpadding="3">
			  <tr>
                ...
                <td class="cobhl" align="center"><strong><a href="javascript:changesortorder('<?php print ($sortorder=='oida'?'oidd':'oida')?>')"><?php print $yyOrdId?></a></strong></td>
				<td class="cobhl" align="left"><strong><a href="javascript:changesortorder('<?php print ($sortorder=='orna'?'ornd':'orna')?>')"><?php print $yyName?></a></strong></td>
				<td class="cobhl" align="center"><strong><a href="javascript:changesortorder('<?php print ($sortorder=='orpa'?'orpd':'orpa')?>')"><?php print $yyMethod?></a></strong></td>
				<td class="cobhl" align="center" width="1%"><strong>AVS</strong></td>
				<td class="cobhl" align="center" width="1%"><strong>CVV</strong></td>
				<td class="cobhl" align="center"><strong><a href="javascript:changesortorder('<?php print ($sortorder=='oraa'?'orad':'oraa')?>')"><?php print $yyAutCod?></a></strong></td>
				<td class="cobhl" align="center"><strong><a href="javascript:changesortorder('<?php print ($sortorder=='orda'?'ordd':'orda')?>')"><?php print $yyDate?></a></strong></td>
