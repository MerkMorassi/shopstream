...
if(@$paypalhostedsolution && @$_GET['tx']!=''){
	...
	if((substr(@$_SERVER['REMOTE_HOST'],0,7)=='216.113' || @$disablepayflowlinkipcheck) && $success && trim(@$_POST['RESULT'])=='0'){
		...
		if(substr(@$_SERVER['REMOTE_HOST'],0,7)!='216.113') $errtext = 'Your IP address of: ' . $_SERVER['REMOTE_HOST'] . ' has been logged.';
		order_failed();
	}
	...
	if(is_numeric($ordID) && getpayprovdetails(16,$data1,$data2,$data3,$demomode,$ppmethod)){
		...
		if(($thesuccess=="approved" || $thesuccess=="submitted") && $theauthcode!=''){
			...
			if($autharr[0]=='Y' && count($autharr) >= 3){
				...
				if($foundorder){
					...
					mysql_query("UPDATE orders SET ordStatus=3,ordAuthStatus='',ordAVS='" . escape_string(substr($theavscode,0,3)) . "',ordCVV='" . escape_string(substr($theavscode,3)) . "',ordAuthNumber='" . substr($theauthcode,0,6) . "',ordTransID='" . substr($theauthcode,6) . "' WHERE ordPayProvider=16 AND ordID='" . escape_string($ordID) . "'") or print(mysql_error());
					order_success($ordID,$emailAddr,$sendEmail);
				}
			}else{
		...
		order_failed();
}elseif(trim(@$_POST['oid'])!='' && trim(@$_POST['response_hash'])!='' && trim(@$_POST['txndate_processed'])!=''){ // Linkpoint 2.0
	$ordID=trim(@$_POST['oid']);
	$ordIDa=explode('.', $ordID);
	$ordID=$ordIDa[0];
	$theauthcode=trim(replace(@$_POST['approval_code'],"'",''));
	$lphash=trim(@$_POST['response_hash']);
	if(is_numeric($ordID) && getpayprovdetails(16,$data1,$data2,$data3,$demomode,$ppmethod)){
		$sSQL = "SELECT ordPrivateStatus FROM orders WHERE ordID='" . escape_string($ordID) . "'";
		$result = mysql_query($sSQL) or print(mysql_error());
		if($rs = mysql_fetch_assoc($result)) $txndatetime=$rs['ordPrivateStatus']; else $txndatetime='';
		mysql_free_result($result);
		$str = $data3 . $theauthcode . @$_POST['chargetotal'] . '840' . $txndatetime . $data1;
		$hex_str='';
		...
		if($ourhash!=$lphash){
			...
			order_failed();
		}elseif(strtoupper(@$_POST['status'])=='APPROVED' || strtoupper(@$_POST['status'])=='SUBMITTED'){
			$autharr = explode(':', $theauthcode);
			if($autharr[0]=='Y' && count($autharr) >= 3){
				$theauthcode = $autharr[1];
				$theavscode = $autharr[2];
