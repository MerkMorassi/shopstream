...
function formvalidator(theForm){
	...
	for(index=0;index<maxrow;index++){
		...
		document.getElementById("optStock" + index).disabled=(document.getElementById("optStock" + index).name=='xxx'?true:false);
		...
		document.getElementById("wsp" + index).disabled=(document.getElementById("wsp" + index).name=='xxx'?true:false);
	}
	return (true);
}
...
<?php			if($adminlanguages>=1 && ($adminlangsettings & 32)==32){
				...
				?></span><span id="swaltlgim<?php print $rowcounter?>" style="display:none"><input type="text" name="<?php print ($optvalue!=''?'altlimg' . $rowcounter:'xxx')?>" id="altlimg<?php print $rowcounter?>" size="20" value="<?php print $optvalue; ?>" onchange="this.name=this.id" /></span></td>
			  </tr>
<?php		} ?>
			</table>
