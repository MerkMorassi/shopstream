...
?>		  </select><?php
		...
		if(($prodfilter & 32)==32){
			...
			print imageorbutton(@$imgfilterproducts,$xxGo,"prodfilter","document.location='".str_replace('&','&amp;',$filterurl)."'+encodeURIComponent(document.getElementById('ectfilter').value)",TRUE);
			print ($usecsslayout ? '</div>' : '</td>');
		}
...
if(mysql_num_rows($allprods) > 0){
	...
	$sSQL = 'SELECT pId,pSKU,'.getlangid('pName',1).','.$WSP.'pPrice,pListPrice,pSection,pSell,pStockByOpts,pStaticPage,pInStock,pExemptions,pTax,pTotRating,pNumRatings,pBackOrder,'.($wantmanufacturer?'mfName,':'').(@$shortdescriptionlimit===0?"'' AS ":'').getlangid('pDescription',2).','.getlangid('pLongDescription',4).' FROM products '.($wantmanufacturer?'LEFT OUTER JOIN manufacturer on products.pManufacturer=manufacturer.mfID ':'').'WHERE pId IN (' . $prodlist . ')' . $sSortBy;
	$allprods = mysql_query($sSQL) or print(mysql_error());
}
$Count = 0;
		if(! @$usecsslayout){
