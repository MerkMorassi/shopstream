...
if(@$_POST['nbx_merchant_reference']!='' && @$_POST['nbx_netbanx_reference']!='' && @$_POST['nbx_checksum']!='' && @$_POST['nbx_status']=='passed'){ // Netbanx
	...
	if(is_numeric($ordID) && getpayprovdetails(15,$data1,$data2,$data3,$demomode,$ppmethod)){
		...
		if($thechecksum==$calculatedsum || trim($data2)==''){
			mysql_query('UPDATE cart SET cartCompleted=1 WHERE cartOrderID=' . $ordID);
			mysql_query("UPDATE orders SET ordAuthNumber='" . escape_string($Txn_id) . "',ordStatus=3,ordAVS='" . escape_string(@$_POST['nbx_houseno_auth'] . '-' . @$_POST['nbx_postcode_auth']) . "',ordCVV='" . escape_string(@$_POST['nbx_CVV_auth']) . "',ordAuthStatus='' WHERE ordPayProvider=15 AND ordID=" . $ordID);
