...
if(frm.amount.value==""){
	alert("<?php print jscheck($xxPlsEntr . ' "' . $xxAmount)?>\".");
	frm.amount.focus();
	return(false);
}
if (!checkastring(frm.amount.value,"0123456789.,")){
	alert("<?php print jscheck($xxOnlyDec . ' "' . $xxAmount)?>\".");
	frm.amount.focus();
	return(false);
}
if(frm.amount.value<<?php print $giftcertificateminimum?>){
	alert("<?php print jscheck($xxGCMini) . ' ' . FormatEuroCurrency($giftcertificateminimum)?>.");
	frm.amount.focus();
	return(false);
}
if(frm.toname.value==""){
	alert("<?php print jscheck($xxPlsEntr . ' "' . $xxTo)?>\".");
	frm.toname.focus();
	return(false);
}
if(frm.fromname.value==""){
	alert("<?php print jscheck($xxPlsEntr . ' "' . $xxFrom)?>\".");
	frm.fromname.focus();
	return(false);
}

if(frm.toemail.value==""){
	alert("<?php print jscheck($xxPlsEntr . ' "' . $xxReEmai)?>\".");
	frm.toemail.focus();
	return(false);
}
var regex = /[^@]+@[^@]+\.[a-z]{2,}$/i;
...
if(frm.toemail2.value!=frm.toemail.value){
	alert("<?php print jscheck($xxEmCNMa)?>.");
	frm.toemail2.focus();
	return(false);
}
return (true);
...
if(frm.gcmessage.value.length>255){
	alert("<?php print jscheck($xxPrd255)?>");
	frm.gcmessage.focus();
	return(false);
}
return (true);
