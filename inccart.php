...
$demomode = FALSE;
$maxshipoptions=40;
$allfreeshipexempt = $success = TRUE;
$packnumber=1;
$fromshipselector = $nodiscounts = $usehst = $multipleoptions = $shiphomecountry = $stockwarning = $backorder = $cartEmpty = $handlingeligableitem = FALSE;
...
function createdynamicstates($sSQL){
	global $origCountry,$origCountryID,$usestateabbrev,$xxPSelUS,$xxPSelCA,$xxPSelUK,$xxPlsSel,$ordState,$ordShipState;
...
?>	function getziptext(cntid){
		if(cntid==1) return("<?php print jsescape($xxZip)?>"); else return("<?php print jsescape($xxPostco)?>");
	}
...
function initshippingmethods(){
	global $shipType,$adminShipping,$adminIntShipping,$allzones,$numzones,$splitUSZones,$shiphomecountry,$numshipoptions,$intShipping,$success,$errormsg,$commercialloc_,$codpaymentprovider,$signaturerelease_,$allowsignaturerelease,$signatureoption,$saturdaydelivery_,$saturdaypickup,$insidedelivery_,$insidepickup,$thesessionid,$willpickup_,$selectedshiptype,$currShipType,$ordShipCity,$countryCurrency,$DHLSiteID,$DHLSitePW,$shipCountryID,$canadapostusername,$canadapostpassword,$packtogether;
	global $uspsmethods,$numuspsmeths,$international,$shipcountry,$maxshipoptions,$origCountry,$willpickuptext,$willpickupcost,$shipstate,$xxNoMeth,$shipinsuranceamt,$fedexaccount,$fedexmeter,$stateAbbrev,$shipStateAbbrev,$usestateabbrev,$ordPayProvider,$adminAltRates,$altrate,$shipping,$willpickupnohandling,$handling,$handlingchargepercent,$shipMethod,$freeshipmethodexists,$multipleoptions,$fedexuserkey,$fedexuserpwd,$fedexpickuptype,$smartPostHub,$smartpostindicia,$smartpostancendorsement;
	global $sXML,$uspsUser,$uspsPw,$upsAccess,$upsUser,$upsPw,$upspickuptype,$origZip,$origCountryCode,$destZip,$shipCountryCode,$adminCanPostUser,$packaging,$adminUnits,$xxPlsSta,$homedelivery,$originstatecode,$ordCity,$ordShipAddress,$ordShipAddress2,$upsnegdrates,$upsnegotiatedaccess,$upsnegotiateduser,$upsnegotiatedpw,$upsAccount,$defaultshipstate,$fromshipselector,$thepweight,$initialpackweight,$iWeight,$combineshippinghandling,$xxShipHa,$xxShippg,$storelang,$splitpackat,$nosplitlargepacks;
	...
	if($fromshipselector){
		...
		$intShipping[0][0]=(@$combineshippinghandling ? $xxShipHa : $xxShippg);
	...
	elseif($shipType==4){
		...
		if(@$canadapostusername!=''){
			$packtogether=TRUE; $splitpackat=''; $nosplitlargepacks=TRUE; // Canada Post Web Services cannot handle more than one package
			$sXML = '<soapenv:Envelope xmlns:rate="http://www.canadapost.ca/ws/soap/ship/rate" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">' .
				'<soapenv:Header><wsse:Security soapenv:mustUnderstand="1" xmlns:wsse="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd" xmlns:wsu="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd"><wsse:UsernameToken><wsse:Username>' . $canadapostusername . '</wsse:Username><wsse:Password>' . $canadapostpassword . '</wsse:Password></wsse:UsernameToken></wsse:Security></soapenv:Header>' .
				'<soapenv:Body><rate:get-rates-request>' . ($storelang=='fr'?'<locale>FR</locale>':'') . '<mailing-scenario><customer-number>' . $adminCanPostUser . '</customer-number><origin-postal-code>' . strtoupper(str_replace(' ','',$origZip)) . '</origin-postal-code><destination>';
...
function splitlargepacks(){
	global $packdims,$shipType,$adminUnits,$nosplitlargepacks,$iWeight,$splitpackat;
	$slpweight = $iWeight;
	$slpnumpacks=1;
	if($shipType==4){
	...
	if(@$nosplitlargepacks!=TRUE && $slpweight > $maxweight && $packdims[7]>1){
		for($slpindex=2; $slpindex<=$packdims[7]; $slpindex++){
			if($slpweight / $slpindex < $maxweight || $slpindex=$packdims[7]){
				$packdims[0] /= $slpindex;
	...
	if($fromshipselector){
				...
				for($index2=0; $index2 < $numzones; $index2++){
					if($allzones[$index2][0] >= $thepweight){
						...
						for($index3=0; $index3 < $numshipoptions; $index3++){
							...
							if($shipCountryCode==$countryCode && $saturdaydelivery_ && ($index3==2 || $index3==3) & @$royalmail){
		...
		if($packtogether){
			...
			if(($prodindex==$itemsincart) && $somethingToShip){
				$numpacks=1;
				if($packdims[7]>1) $numpacks *= splitlargepacks();
				if($international!='')
...
function calculateshipping(){
	global $shipType,$freeshipmethodexists,$multipleoptions,$somethingToShip,$willpickuptext,$willpickupcost,$allzones,$numshipoptions,$upsUser,$upsPw,$shipCountryCode,$destZip,$totalgoods,$thesessionid,$handling,$selectedshiptype,$willpickup_,$DHLAccountNo,$origCountryCode,$countryCurrency,$canadapostusername;
	global $shipping,$shipMethod,$success,$errormsg,$xxNoMeth,$sXML,$intShipping,$international,$iTotItems,$uspsmethods,$numuspsmeths,$shipstate,$maxshipoptions,$saturdaydelivery_,$saturdaypickup,$upsnegdrates,$fromshipselector,$packageweight,$packagefreeexemptweight,$shipCountryID,$ordCity;
	if($fromshipselector){
	}elseif($shipType==1){
		...
		$sXML .= '</Pieces><PaymentAccountNumber>' . $DHLAccountNo . '</PaymentAccountNumber><IsDutiable>' . ($origCountryCode==$shipCountryCode ? 'N' : 'Y') . '</IsDutiable>' .
			'<NetworkTypeCode>AL</NetworkTypeCode></BkgDetails><To><CountryCode>' . $shipCountryCode . '</CountryCode><Postalcode>' . $destZip . '</Postalcode>' . (zipisoptional($shipCountryID)?'<City>'.$ordCity.'</City>':'') . '</To>' .
...
}elseif($checkoutmode=='paypalexpress1'){ // PayPal Express
	if(getpayprovdetails(19,$username,$password,$data3,$demomode,$ppmethod)){
		$data2arr = explode('&',$password);
		$password=urldecode(@$data2arr[0]);
		$isthreetoken=(trim(urldecode(@$data2arr[2]))=='1');
...
<?php
	...
	<input type="hidden" name="saddaddress" id="saddaddress" value="<?php print ($numaddresses>0 ? '' : 'add')?>" />
			  <table class="cobtbl" width="100%" border="0" cellspacing="1" cellpadding="3" style="text-align:<?php print $tleft?>;">
				<tr>
				  <td class="cobhl" align="center"<?php print $colspan4?> height="34"><strong><?php print $xxCstDtl?></strong></td>
				</tr>
<?php
	function writeshippingflags($colspan){
		global $willpickuptext,$willpickupcost,$commercialloc,$saturdaydelivery,$addshippinginsurance,$allowsignaturerelease,$signatureoption,$insidedelivery,$ordComLoc,$xxComLoc,$xxSatDel,$xxWantIns,$xxSigRel,$xxInsDel,$redstar,$holdatlocation,$homedelivery,$tleft,$tright,$ordName,$inlinecheckout;
		$totcols=($inlinecheckout?2:4);
		if(@$willpickuptext!=''){ ?>
			<tr><td align="<?php print $tright?>" class="cobhl" colspan="<?php print $colspan?>"><input type="checkbox" name="willpickup" value="Y" /></td>
			<td class="cobll" colspan="<?php print $totcols-$colspan?>"><span style="font-size:10px"><?php print $willpickuptext . (@$willpickupcost!=''?' (' . FormatEuroCurrency($willpickupcost) . ')' : '')?></span></td></tr>
<?php	}
		if(@$commercialloc==TRUE){ ?>
			...
			<td class="cobll" colspan="<?php print $totcols-$colspan?>"><span style="font-size:10px"><?php print $xxComLoc?></span></td></tr>
<?php	}
		if(@$saturdaydelivery==TRUE){ ?>
			...
			<td class="cobll" colspan="<?php print $totcols-$colspan?>"><span style="font-size:10px"><?php print $xxSatDel?></span></td></tr>
<?php	}
		if(abs(@$addshippinginsurance)==2){ ?>
			...
			<td class="cobll" colspan="<?php print $totcols-$colspan?>"><span style="font-size:10px"><?php print $xxWantIns?></span></td></tr>
<?php	}
		if(@$allowsignaturerelease==TRUE && @$signatureoption!=''){ ?>
			...
			<td class="cobll" colspan="<?php print $totcols-$colspan?>"><span style="font-size:10px"><?php print $xxSigRel?></span></td></tr>
<?php	}
		if(@$insidedelivery==TRUE){ ?>
			...
			<td class="cobll" colspan="<?php print $totcols-$colspan?>"><span style="font-size:10px"><?php print $xxInsDel?></span></td></tr>
<?php	}
		if(@$holdatlocation==TRUE){ ?>
			...
			<td class="cobll" colspan="<?php print $totcols-$colspan?>"><span style="font-size:10px">Please click here to Hold at Location</span></td></tr>
<?php	}
		if(@$homedelivery==TRUE){ ?>
			<tr><td align="<?php print $tright?>" class="cobhl" colspan="<?php print $colspan?>">Delivery Options:</td>
			<td class="cobll" colspan="<?php print $totcols-$colspan?>"><select name="homedelivery" size="1">
...
<?php	function writeaddressspans($isshp){
			global $useaddressline2,$extraorderfield1html,$extraorderfield2html,$extraorderfield1required,$extraorderfield2required,$extraorderfield1,$extraorderfield2,$nonhomecountries,$numaddresses,$addresses,$xxSamAs,$xxName,$xxFirNam,$xxLasNam,$xxAddress,$xxAddress2,$xxCity,$xxState,$xxStaPro,$xxCountry,$xxZip,$xxPhone,$xxNew,$xxEdit,$xxCancel,$usefirstlastname,$redstar,$tleft,$tright,$hasstates,$colspan2;
...
<?php	} ?>
		...
		if(@$usefirstlastname){
			...
			print '<input type="text" name="name" size="20" value="' . htmlspecials(@$ordName) . '" />'; ?></td><?php print $linebreak?>
...
<?php	} ?>
				...
				<?php print $linebreak?>
				  <td align="<?php print $tright?>" class="cobhl"><strong><?php print replace($redstar,'<span','<span id="zipstar"')?><span id="ziptxt"><?php print $xxZip?></span>:</strong></td>
				...
				<tr>
				  ...
				  <td class="cobll"<?php	if(trim(@$extraorderfield2)=='') print $colspan3;?>><input type="text" name="phone" size="20" value="<?php print @$ordPhone?>" alt="<?php print $xxPhone?>" /></td>
<?php	if(trim(@$extraorderfield2)!=''){
			print $linebreak; ?>
...
<?php	writeshippingflags(1);
		if(@$noshipaddress!=TRUE){ ?>
				<tr>
				  <td align="center"<?php print $colspan4?> class="cobhl" height="30"><strong><?php if(@$hideshipaddress!=TRUE){ print $xxShpDiff; $shprowstyle=''; }else{ print $xxShpDff . '<input type="checkbox" name="shipdiff" id="shipdiff" value="1" onclick="showshipform(this)" style="vertical-align:middle;" />'; $shprowstyle=' style="display:none"'; } ?></strong></td>
				</tr>
...
<?php		} ?>
				...
				<?php print $linebreak?>
				  <td align="<?php print $tright?>" class="cobhl"><strong><?php print replace($redstar,'<span','<span id="szipstar"')?><span id="sziptxt"><?php print $xxZip?></span>:</strong></td>
				...
				<tr class="shipformrow"<?php print $shprowstyle?>>
				  ...
				  <td class="cobll"<?php	if(trim(@$extraorderfield2)=='') print $colspan3?>><input type="text" name="sphone" size="20" value="<?php print @$ordShipPhone?>" alt="<?php print $xxPhone?>" /></td>
<?php		if(trim(@$extraorderfield2)!=''){
				print $linebreak; ?>
...
<?php	} // noshipaddress
				...
				<tr><td class="cobhl" align="center"<?php print $colspan4?> height="30"><strong><?php print $xxMisc?></strong></td></tr>
				<tr>
				  <td align="<?php print $tright?>" class="cobhl"><strong><?php print $xxAddInf?>.</strong></td>
				  <td class="cobll"<?php print $colspan3?>><textarea name="ordAddInfo" class="addinfo" rows="3" cols="44"><?php print @$ordAddInfo?></textarea></td>
				</tr>
<?php
...
<?php	} ?>}
if(frm.zip.value=="" && ! zipoptional(cntelem)){
	alert("<?php print jscheck($xxPlsEntr)?> \"" + getziptext(cntelem[cntelem.selectedIndex].value) + "\".");
	frm.zip.focus();
	return(false);
}
if(frm.phone.value==""){
...
<?php	} ?>
	...
	if(frm.szip.value=="" && ! zipoptional(scntelem)){
		alert("<?php print jscheck($xxShpDtls)?>\n\n<?php print jscheck($xxPlsEntr)?> \"" + getziptext(scntelem[scntelem.selectedIndex].value) + "\".");
		frm.szip.focus();
		return(false);
	}
...
?>/* ]]> */</script><?php
	...
	if(@$_POST['orderid']!='' && is_numeric(@$_POST['orderid']) && @$_POST['sessionid']!=''){
		...
		if(@$_SESSION['clientID']!=''){
			if(@$_POST['addaddress']=='add'){
				...
				$result = mysql_query($sSQL) or print(mysql_error());
				$hasaddress = (mysql_num_rows($result)>0);
				mysql_free_result($result);
				...
				if(! $hasaddress) mysql_query($sSQL) or print(mysql_error());
	...
	if($success && (($itemsincart=mysql_num_rows($allcart))>0)){
		...
		do_stock_check(FALSE,$backorder,$stockwarning);
		if(@$_POST['shipselectoraction']!=''){ $stockwarning=FALSE; $backorder=FALSE; }
		if($stockwarning){
			$checkoutmode='';
			if(ob_get_length()!==FALSE)
				header('Location: ' . $storeurl . 'cart.php');
	...
	if($stockwarning){
		...
		if($success==FALSE){
			...
			if($data3!=''){
				...
				mysql_query("UPDATE orders SET ordPrivateStatus='" . escape_string($formattedDate) . "' WHERE ordID='" . escape_string($orderid) . "'") or print(mysql_error());
...
<?php	}else{ ?>
	  <form method="post" action="cart.php" name="checkoutform">
		...
		<table class="cobtbl" width="100%" border="0" cellspacing="1" cellpadding="3">
		  ...
		  <tr>
			<td class="cobll" colspan="2" height="30" align="center">&nbsp;<br /><?php print imageorsubmit(@$imggoback, $xxGoBack, 'gobackbutton')?><br />&nbsp;</td>
		  </tr>
		</table>
	  </form>
<?php	}
...
?>				  <tr style="height:30px;font-weight:bold;">
					<td class="cobhl cobcol1" align="left"><?php print $xxCODets?></td>
...
<?php	}
	...
	if($checkoutmode=='dologin' || ($checkoutmode=='donewaccount' && @$allowclientregistration==TRUE)){
		...
		if($loginsuccess){
			...
			if($rs = mysql_fetch_assoc($result)){
				...
				$_SESSION['clientPercentDiscount']=(100.0-(double)$rs['clPercentDiscount'])/100.0;
				get_wholesaleprice_sql();
				ectsetcookie('WRITECLL', $clientEmail, @$_POST['cook']=='ON'?365:0, '/', '');
				ectsetcookie('WRITECLP', $clientPW, @$_POST['cook']=='ON'?365:0, '/', '');
				$loginsuccess=TRUE;
			}else
				$loginerror=$xxNoLogD;
		}
	...
	if(@$estimateshipping==TRUE){
		...
		if($wantstateselector){
			...
			if(mysql_num_rows($result)==0){ $wantstateselector=FALSE; $splitUSZones=FALSE; }
			mysql_free_result($result);
		}
	...
	if($stockalreadysubtracted) $stockwarning=FALSE; else $stockwarning=do_stock_check(TRUE,$backorder,$stockwarning);
	$alldata='';
...
<?php	$totaldiscounts = 0;
		...
		while($alldata=mysql_fetch_assoc($result)){
			...
			while($rs=mysql_fetch_assoc($opts)){
				...
				$theoptions .= '<tr><td class="cobhl cobcol1" align="' . $tright . '" height="25"><span style="font-size:10px"><strong>' . $rs['coOptGroup'] . ':</strong></span></td>' .
