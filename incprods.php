		...
		for($wmlindex=0; $wmlindex < $numcats; $wmlindex++){
			if($allcatsa[$wmlindex][2]==$id){
				...
				for($index = 0; $index < $itlevel-1; $index++)
					print $raquo . ' ';
				print htmldisplay($allcatsa[$wmlindex][1]) . "</option>\n";
...
if(@$_POST['posted']=='1'){
	$pExemptions=0;
	$newid=trim(unstripslashes(@$_POST['newid']));
	...
	if(@$_POST['act']=='allstk'){
		...
		dodeleteprod(unstripslashes(@$_POST['id']));
		$dorefresh=TRUE;
		...
		if($newid!=trim(@$_POST['id'])){
			if(strtolower($newid)==strtolower(trim(@$_POST['id'])))
				$success=TRUE;
			else{
				...
				mysql_free_result($result);
				if($success){
					mysql_query("UPDATE pricebreaks SET pbProdID='" . escape_string($newid) . "' WHERE pbProdID='" . escape_string(unstripslashes(@$_POST['id'])) . "'") or print(mysql_error());
					mysql_query("UPDATE cpnassign SET cpaAssignment='" . escape_string($newid) . "' WHERE cpaType=2 AND cpaAssignment='" . escape_string(unstripslashes(@$_POST['id'])) . "'") or print(mysql_error());
					mysql_query("UPDATE relatedprods SET rpProdID='" . escape_string($newid) . "' WHERE rpProdID='" . escape_string(unstripslashes(@$_POST['id'])) . "'") or print(mysql_error());
					mysql_query("UPDATE relatedprods SET rpRelProdID='" . escape_string($newid) . "' WHERE rpRelProdID='" . escape_string(unstripslashes(@$_POST['id'])) . "'") or print(mysql_error());
					mysql_query("UPDATE ratings SET rtProdID='" . escape_string($newid) . "' WHERE rtProdID='" . escape_string(unstripslashes(@$_POST['id'])) . "'") or print(mysql_error());
				}
			}
		}
		if($success){
			...
			$sSQL = "UPDATE products SET ";
						...
						$sSQL .= 'pGiftWrap=' . (trim(@$_POST['pGiftWrap']) == '1' ? 1 : 0) . ',';
						$sSQL .= 'pBackOrder=' . (trim(@$_POST['pBackOrder']) == '1' ? 1 : 0) . ',';
			...
			for($rowcounter=0; $rowcounter < maxprodopts; $rowcounter++){
				if(@$_POST['pOption' . $rowcounter]!='' && @$_POST['pOption' . $rowcounter]!=0){
					$sSQL = "INSERT INTO prodoptions (poProdID,poOptionGroup) VALUES ('" . escape_string($newid) . "'," . @$_POST['pOption' . $rowcounter] . ')';
					mysql_query($sSQL) or print(mysql_error());
				}
			}
			...
			for($rowcounter=0; $rowcounter < $maxprodsects; $rowcounter++){
				if(@$_POST['pSection' . $rowcounter]!='' && @$_POST['pSection' . $rowcounter]!=0 && @$_POST['pSection'] != @$_POST['pSection' . $rowcounter]){
					$sSQL = "INSERT INTO multisections (pID,pSection) VALUES ('" . escape_string($newid) . "'," . @$_POST['pSection' . $rowcounter] . ')';
					mysql_query($sSQL) or print(mysql_error());
				}
			}
			if(is_numeric(trim(@$_POST['inStock']))){
				if((int)@$_POST['inStock']>0){
					checknotifystock($newid);
				}
			}
			$dorefresh=TRUE;
		}else
		...
		if($success){
			...
			$sSQL .= 'pStockByOpts,pStaticPage,pRecommend,pGiftWrap,pBackOrder,pSell,pWeight) VALUES (';
						...
						$sSQL .= (trim(@$_POST['pGiftWrap']) == '1' ? 1 : 0) . ',';
						$sSQL .= (trim(@$_POST['pBackOrder']) == '1' ? 1 : 0) . ',';
			...
			$sSQL = "DELETE FROM multisections WHERE pID='" . escape_string($newid) . "'";
			mysql_query($sSQL) or print(mysql_error());
			for($rowcounter=0; $rowcounter < $maxprodsects; $rowcounter++){
				if(@$_POST['pSection' . $rowcounter]!='' && @$_POST['pSection' . $rowcounter]!=0 && @$_POST['pSection'] != @$_POST['pSection' . $rowcounter]){
					$sSQL = "INSERT INTO multisections (pID,pSection) VALUES ('" . escape_string($newid) . "'," . @$_POST['pSection' . $rowcounter] . ')';
	...
	if($success && (@$_POST['act']=='domodify' || @$_POST['act']=='doaddnew')){
		...
		for($index=0; $index<=$maximgindex; $index++){
			if(@$_POST['smim' . $index]!='') mysql_query("INSERT INTO productimages (imageProduct,imageSrc,imageNumber,imageType) VALUES ('" . escape_string($newid) . "','" . escape_string(@$_POST['smim' . $index]) . "'," . $index . ",0)") or print(mysql_error());
			if(@$_POST['lgim' . $index]!='') mysql_query("INSERT INTO productimages (imageProduct,imageSrc,imageNumber,imageType) VALUES ('" . escape_string($newid) . "','" . escape_string(@$_POST['lgim' . $index]) . "'," . $index . ",1)") or print(mysql_error());
			if(@$_POST['gtim' . $index]!='') mysql_query("INSERT INTO productimages (imageProduct,imageSrc,imageNumber,imageType) VALUES ('" . escape_string($newid) . "','" . escape_string(@$_POST['gtim' . $index]) . "'," . $index . ",2)") or print(mysql_error());
		}
	}
...
<script language="javascript" type="text/javascript" src="popcalendar.js">
	...
	<form name="mainform" method="post" action="adminprods.php" onsubmit="return formvalidator(this)">
            ...
            <table id="producttable" width="100%" border="0" cellspacing="0" cellpadding="3">
			  ...
			  <tr>
			    <td align="right"><?php print $redasterix.$yyPrRef?>:</td><td><input type="text" name="newid" size="25" value="<?php print htmlspecials($pId)?>" /></td>
			  ...
			  <tr>
			    <td align="right"><?php print $redasterix.$yyPrNam?>:</td><td><input type="text" name="pName" size="40" value="<?php print htmlspecialsucode($pName)?>" /></td>
			    <td align="right"><?php print $redasterix.$yyPrPri?>:</td><td><input type="text" name="pPrice" size="15" value="<?php print $pPrice?>" /></td>
			  </tr>
<?php		for($index=2; $index <= $adminlanguages+1; $index++){
				if(($adminlangsettings & 1)==1){
			?><tr>
			    <td align="right"><?php print $redasterix.$yyPrNam . " " . $index?>:</td><td colspan="3"><input type="text" name="pName<?php print $index?>" size="40" value="<?php print htmlspecialsucode($pNames[$index])?>" /></td>
			  </tr><?php
				}
			} ?>
			  <tr>
...
<?php	if($simpleOptions){
			...
			if(($index % 2)==0)
			  ...
			  <tr> 
                ...
                <td colspan="2"><textarea name="pDescription" cols="55" rows="8"><?php print htmlspecialsucode($pDescription)?></textarea></td>
				<td align="center">
...
<?php	for($index=2; $index <= $adminlanguages+1; $index++){
			if(($adminlangsettings & 2)==2){ ?>
			  <tr>
                ...
                <td colspan="3"><textarea name="pDescription<?php print $index?>" cols="55" rows="8"><?php print htmlspecialsucode($pDescriptions[$index])?></textarea></td>
			  </tr>
<?php		}
		} ?>
			  <tr>
                <td align="right"><?php print $yyLnDesc?>:</td>
                <td colspan="3" align="left"><textarea name="pLongDescription" cols="65" rows="9"><?php print htmlspecialsucode($pLongDescription)?></textarea></td>
			  </tr>
<?php	for($index=2; $index <= $adminlanguages+1; $index++){
			if(($adminlangsettings & 4)==4){ ?>
			  <tr>
				<td align="right"><?php print $yyLnDesc . ' ' . $index?>:</td>
                <td colspan="3"><textarea name="pLongDescription<?php print $index?>" cols="65" rows="9"><?php print htmlspecialsucode($pLongDescriptions[$index])?></textarea></td>
			  </tr>
<?php		}
		} ?>
			  <tr>
			  ...
			  <tr>
				<td align="right"><?php print $yyGifWra?>:</td>
                <td><input type="checkbox" name="pGiftWrap" value="1"<?php if((int)$pGiftWrap!=0) print ' checked="checked"' ?> /></td>
				<td align="right"><?php print $yyBakOrd?>:</td>
                <td><input type="checkbox" name="pBackOrder" value="1"<?php if((int)$pBackOrder!=0) print ' checked="checked"' ?> /></td>
			  </tr>
			  <tr>
...
<?php
	if(@$htmleditor=='fckeditor' || @$htmleditor=='ckeditor'){
		...
		if($htmleditor=='ckeditor'){
			$streditor = "var pDescription=CKEDITOR.replace('pDescription',{extraPlugins : 'autogrow,stylesheetparser',autoGrow_maxHeight : 800,removePlugins : 'resize', toolbarStartupExpanded : false, toolbar : 'Basic', filebrowserBrowseUrl :'ckeditor/filemanager/browser/default/browser.html?Connector=".$storeurl."vsadmin/ckeditor/filemanager/connectors/php/connector.php',filebrowserImageBrowseUrl : 'ckeditor/filemanager/browser/default/browser.html?Type=Image&Connector=".$storeurl."vsadmin/ckeditor/filemanager/connectors/php/connector.php',filebrowserFlashBrowseUrl :'ckeditor/filemanager/browser/default/browser.html?Type=Flash&Connector=".$storeurl."vsadmin/ckeditor/filemanager/connectors/php/connector.php',filebrowserUploadUrl:'".$storeurl."vsadmin/ckeditor/filemanager/connectors/php/upload.php?Type=File',filebrowserImageUploadUrl:'".$storeurl."vsadmin/ckeditor/filemanager/connectors/php/upload.php?Type=Image',filebrowserFlashUploadUrl:'".$storeurl."vsadmin/ckeditor/filemanager/connectors/php/upload.php?Type=Flash'});\r\n";
...
/* ]]> */
		...
		<form name="mainform" method="post" action="adminprods.php">
		  <td width="100%">
			...
			<input type="hidden" name="id" value="<?php print htmlspecials(@$_POST['id'])?>" />
		...
		if($numcats > 0){
			if(@$adminonlysubcats==TRUE){
				for($index=0;$index<$numcats;$index++){
					...
					print '>' . htmldisplay($allcatsa[$index][1]) . "</option>\n";
				}
			}else
				writemenulevel(0,1);
		}
...
<?php	} ?>
			  ...
			  <tr>
					...
					if(@$_POST['act']=='search' || @$_GET['pg']!=''){
						if($pract=='del' || $pract=='dip' || $pract=='stp' || $pract=='rec' || $pract=='gwr' || $pract=='bak' || $pract=='ste' || $pract=='cte' || $pract=='she' || $pract=='hae' || $pract=='fse'){ ?>
	...
	if(@$_POST['act']=='search' || @$_GET['pg']!=''){
		...
		function displayprodrow($xrs,$rownum){
			...
			if($stockManage!=0){
				...
				elseif($pract=='gwr')
					print '<input type="hidden" name="pra_'.patch_pid($xrs['pID']).'" value="1" /><input type="checkbox" id="chkbx'.$resultcounter.'" name="prb_'.patch_pid($xrs['pID']).'" value="1" ' . ($xrs['pGiftWrap']!=0 ? 'checked="checked"' : '') . 'tabindex="'.($resultcounter+1).'"/>';
				elseif($pract=='bak')
					print '<input type="hidden" name="pra_'.patch_pid($xrs['pID']).'" value="1" /><input type="checkbox" id="chkbx'.$resultcounter.'" name="prb_'.patch_pid($xrs['pID']).'" value="1" ' . ($xrs['pBackOrder']!=0 ? 'checked="checked"' : '') . 'tabindex="'.($resultcounter+1).'"/>';
				elseif($pract=='sku')
				...
				elseif($pract=='ppt')
					print '<input type="text" id="chkbx'.$resultcounter.'" size="5" name="pra_'.patch_pid($xrs['pID']).'" value="' . $xrs['pTax'] . '" tabindex="'.($resultcounter+1).'"/>';
				elseif($pract=='sel')
				...
				elseif($pract=='ste' || $pract=='cte' || $pract=='she' || $pract=='hae' || $pract=='fse'){
					...
					print '<input type="text" id="chkbx'.$resultcounter.'" size="5" name="pra_'.patch_pid($xrs['pID']).'" value="' . $xrs['pShipping'] . '" tabindex="'.($resultcounter+1).'"/> / <input type="text" id="chkbx'.$resultcounter.'" size="5" name="prb_'.patch_pid($xrs['pID']).'" value="' . $xrs['pShipping2'] . '" tabindex="'.($resultcounter+1).'"/>';
				else
					print '&nbsp;';
			?></td><td><?php print $xrs['pID']?></td><td><?php
			...
			?>type="button" value="<?php print $yyAssign?>" onclick="dsc('<?php print jsspecials($xrs['pID'])?>')" /></td><td><input type="button" value="<?php print $yyModify?>" onclick="mrk('<?php print jsspecials($xrs['pID'])?>',event)" /></td><?php
				if($rid!=''){
			...
			?><td><input type="button" id="rel<?php print htmlspecials($xrs['pID'])?>" value="<?php print $yyRelate?>" onclick="rel('<?php print jsspecials($xrs['pID'])?>')" /></td><?php
				}
			?><td><input type="button" value="<?php print $yyDelete?>" onclick="drk('<?php print jsspecials($xrs['pID'])?>')" /></td></tr><?php
			print "\r\n";
			$resultcounter++;
		}
		function displayheaderrow(){
			global $yyPrId,$yyPrName,$yyDiscnt,$yyModify,$yyRelate,$yyDelete,$yyStck,$useStockManagement,$pract,$adminlangsettings,$adminlanguages,$perproducttaxrate,
			$yyPrPri,$yyWhoPri,$yyListPr,$yyStck,$yyDelete,$yyPrWght,$yyDisPro,$yyStatPg,$yyRecomd,$yyProdOr,$yySellBut,$yyGifWra,$yyBakOrd,$yyTax; ?>
			<tr>
...
<?php	}
		...
		mysql_free_result($result);
		$columnlist='products.pID,pName,pName2,pName3,pDisplay,pSell,pExemptions,pShipping,pShipping2,pInStock,rootSection,pStockByOpts,pPrice,pWholesalePrice,pListPrice,pOrder,pRecommend,pGiftWrap,pBackOrder,pStaticPage,pSKU,pWeight,pTax';
		if(@$_GET['related']=='go'){
		...
		if($pidlist!='' && $rid==''){
			...
			print '</script>';
			  ...
			  <tr>
                ...
                <td width="100%" colspan="6" align="center"><br /><a href="admin.php"><strong><?php print $yyAdmHom?></strong></a><br />&nbsp;<br />
