...
if(frm.affilid.value==""){
	alert("<?php print jscheck($xxPlsEntr . ' "' . $xxAffID)?>\".");
	frm.affilid.focus();
	return (false);
}
...
if (!allValid){
    alert("<?php print jscheck($xxAlphaNu . ' "' . $xxAffID)?>\".");
    frm.affilid.focus();
    return (false);
}
...
if(frm.state.value==""){
	alert("<?php print jscheck($xxPlsEntr . ' "' . $xxAllSta)?>\".");
	frm.state.focus();
	return (false);
}
if(frm.zip.value==""){
...
?>
				...
				<tr>
				  <td width="50%" align="center" colspan="4"><?php
					print imageorsubmit(@$imgsubmit,$xxSubmt,'');
					if(@$_POST['act']=='editaffil' && trim(@$_SESSION['xaffilid'])!=''){
