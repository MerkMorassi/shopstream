...
if(@$_POST['posted']=='1' || @$_GET['pg']!=''){
	...
	if(mysql_num_rows($allprods) == 0)
		...
		$sSQL = 'SELECT pId,pSKU,'.getlangid('pName',1).','.$WSP.'pPrice,pListPrice,pSection,pSell,pStockByOpts,pStaticPage,pInStock,pExemptions,pTax,pTotRating,pNumRatings,pBackOrder,'.($wantmanufacturer?'mfName,':'').getlangid('pDescription',2).','.getlangid('pLongDescription',4).' FROM products '.($wantmanufacturer?'LEFT OUTER JOIN manufacturer on products.pManufacturer=manufacturer.mfID ':'').'WHERE pId IN (' . $prodlist . ')' . $sSortBy;
		$allprods = mysql_query($sSQL) or print(mysql_error());
	}
}
if($nobox==''){
...
<?php
	if(@$searchbymanufacturer!=''){ ?>
		...
		mysql_free_result($result2); ?></select></td>
			    <td class="cobll" colspan="2" align="center"><?php print imageorsubmit(@$imgsearch,$xxSearch,'')?></td>
			  </tr>
<?php
	}else{ ?>
			  <tr>
			    <td class="cobhl">&nbsp;</td>
			    <td class="cobll" colspan="3" align="center"><?php print imageorsubmit(@$imgsearch,$xxSearch,'')?></td>
			  </tr>
<?php
	} ?>
			</table>
...
if(@$_POST['posted']=='1' || @$_GET['pg']!=''){
	if(! @$usecsslayout){
		...
		if(! $success)
			print '<tr><td align="center"><p>&nbsp;</p><p><strong>' . $xxSrchNM . '</strong></p><p>&nbsp;</p></td></tr>';
		else
			print '<tr><td width="100%">';
	}else{
		print '<br /><div>';
		if(! $success)print '<div class="nosearchresults">' . $xxSrchNM . '</div>';
	}
	if($success){
