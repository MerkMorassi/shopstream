					...
					if($menustyle=='horizontalmenu1' || $menustyle=='verticalmenu3'){
						if(! (@$menucategoriesatroot===TRUE && $mAlldata[$mIndex][0]==$catalogroot)){
							...
							if(trim($mAlldata[$mIndex][4])!=''){
								print '<a href="'.(strpos($mAlldata[$mIndex][4],'://')===FALSE?$incstoreurl:'').$mAlldata[$mIndex][4].'">'.str_replace('<','&lt;',$mAlldata[$mIndex][1])."</a>\r\n";
							}else{
						...
						if(trim($mAlldata[$mIndex][4])!=''){
							print $menuheadsec.'"products' . $mAlldata[$mIndex][0] . '","' . @$menuprestr . str_replace('"','\"',$mAlldata[$mIndex][1]) . @$menupoststr . '","' . (strpos($mAlldata[$mIndex][4],'://')===FALSE?$incstoreurl:'') . $mAlldata[$mIndex][4] . "\");\n";
						}else{
							if($mAlldata[$mIndex][3]==0)
	...
	if($itlevel<=$menupoplimit){
		for($mIndex=0;$mIndex < $numrows;$mIndex++){
			if($mAlldata[$mIndex][2]==$id && ! (@$menucategoriesatroot===TRUE && $mAlldata[$mIndex][0]==$catalogroot)){
				$jsstr.='em['.$mAlldata[$mIndex][0].']='.$mAlldata[$mIndex][2].';';
				if(trim($mAlldata[$mIndex][4])!=''){
					$mlink = (strpos($mAlldata[$mIndex][4],'://')===FALSE?$incstoreurl:'') . $mAlldata[$mIndex][4];
				}else{
					if($mAlldata[$mIndex][3]==0)
...
function displayectmenu($menstyle){
	global $jsstr,$menupoplimit,$minloglevel,$numrows,$mAlldata,$menuid,$menustyle,$menucategoriesatroot,$catalogroot,$alreadygotadmin,$sortcategoriesalphabetically,$incstoreurl,$storeurl;
