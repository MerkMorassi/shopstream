...
function execute_sql(){
	...
	if($isimagesupdate){
		...
		if(mysql_affected_rows() == 0){
			...
			if($pimage!='') mysql_query("UPDATE productimages SET imageSrc='" . escape_string($pimage) . "' WHERE imageProduct='" . escape_string($valuesarray[$keycolumn]) . "' AND imageNumber=0 AND imageType=0") or csv_database_error();
			if($plargeimage!='') mysql_query("UPDATE productimages SET imageSrc='" . escape_string($plargeimage) . "' WHERE imageProduct='" . escape_string($valuesarray[$keycolumn]) . "' AND imageNumber=0 AND imageType=1") or csv_database_error();
		}
	}else{
		$hasdimensions=FALSE;
		$dimspattern='PLENxPWIDxPHEI';
		...
		if($pimage!='') mysql_query("INSERT INTO productimages (imageProduct,imageSrc,imageNumber,imageType) VALUES ('" . escape_string($pid) . "','" . escape_string($pimage) . "',0,0)") or csv_database_error();
		if($plargeimage!='') mysql_query("INSERT INTO productimages (imageProduct,imageSrc,imageNumber,imageType) VALUES ('" . escape_string($pid) . "','" . escape_string($plargeimage) . "',0,1)") or csv_database_error();
	}
}
...
function getmanufacturer($swn){
	$sSQL = "SELECT mfID FROM manufacturer WHERE mfName='".escape_string($swn)."'";
	$result2 = mysql_query($sSQL) or mysql_error();
	if($rs2 = mysql_fetch_assoc($result2)) $secid=$rs2['mfID']; else $secid=0;
	mysql_free_result($result2);
	return($secid);
}
if(@$_POST["posted"]=="1"){
	...
	if($csvfile['error']=='1'){
		...
		elseif($column_list=='pid,pname,pprice,pinstock,optid,optiongroup,option')
			...
			mysql_query('SELECT ' . $column_list_copy . " FROM products WHERE pID='abcwxyz'") or toplevel_database_error($column_list_copy);
		}
		if($success){
