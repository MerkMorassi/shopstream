...
function do_order_success($sorderid,$sEmail,$sendstoreemail,$doshowhtml,$sendcustemail,$sendaffilemail,$sendmanufemail){
	...
	if(mysql_num_rows($result) > 0){
		...
		if($rs['ordShipType']=='MODWARNOPEN'){
			...
			print imageorbutton(@$imgcontinueshopping,'&nbsp;'.$xxCntShp.'&nbsp;','',(@$thankspagecontinue!=''?$thankspagecontinue:$storeurl), (@$thankspagecontinue=='javascript:history.go(-1)'?TRUE:FALSE)).'&nbsp;';
			print '</div>';
			$success=FALSE;
			mysql_free_result($result);
			return;
