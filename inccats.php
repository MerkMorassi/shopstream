...
if(@$_POST['act']=='changepos'){
	...
	if(@$_POST['act']=='delete'){
		...
		$sSQL .= ",sectionHeader='" . escape_string(unstripslashes(@$_POST['sectionHeader'])) . "'";
		for($index=2; $index <= $adminlanguages+1; $index++){
			if(($adminlangsettings & 524288)==524288) $sSQL .= ",sectionHeader" . $index . "='" . escape_string(unstripslashes(@$_POST['sectionHeader' . $index])) . "'";
		}
		...
		$sSQL .= ',sectionDisabled,sectionHeader';
		for($index=2; $index <= $adminlanguages+1; $index++){
			if(($adminlangsettings & 524288)==524288) $sSQL .= ',sectionHeader' . $index;
		}
		$sSQL .= ",sectionurl) VALUES ('" . escape_string(unstripslashes(@$_POST["secname"])) . "','" . escape_string(unstripslashes(@$_POST["secdesc"])) . "','" . escape_string(unstripslashes(@$_POST["secimage"])) . "'," . $mxOrder . "," . @$_POST["tsTopSection"] . "," . @$_POST["catfunction"];
		$workname = escape_string(unstripslashes(@$_POST["secworkname"]));
		if($workname!='')
			$sSQL .= ",'" . $workname . "'";
		else
		...
		$sSQL .= ",'" . escape_string(unstripslashes(@$_POST['sectionHeader'])) . "'";
		for($index=2; $index <= $adminlanguages+1; $index++){
			if(($adminlangsettings & 524288)==524288) $sSQL .= ",'" . escape_string(unstripslashes(@$_POST['sectionHeader' . $index])) . "'";
		}
...
<?php	}
		...
		if(@$_POST["act"]=="modify" && is_numeric(@$_POST["id"])){
			...
			$sSQL .= ',sectionHeader,sectionHeader2,sectionHeader3 FROM sections WHERE sectionID=' . @$_POST['id'];
			$result = mysql_query($sSQL) or print(mysql_error());
			if($rs = mysql_fetch_assoc($result)){
...
<?php		}
			...
			<tr>
			  <td align="right">Catalog Root:</td>
			  <td><input type="checkbox" name="catalogroot" value="ON" <?php if($catalogroot==$sectionID) print 'checked="checked" '?>/> (Optional) Check to make this category the product catalog root.</td>
			</tr>
			<tr id="sectionHeadTR">
