...
if(@$_POST['mode']!='checkout' && @$_POST['mode']!='add' && @$_POST['mode']!='go' && @$_POST['mode']!='paypalexpress1' && @$_POST['mode']!='authorize'){
	...
	for($csindex=0; $csindex < count($crosssellactionarr); $csindex++){
		...
		if($relatedlist!=''){
			...
			$sSQL = 'SELECT pId,pSKU,' . getlangid('pName',1) . ',' . $WSP . 'pPrice,pListPrice,pSection,pSell,pStockByOpts,pStaticPage,pInStock,pExemptions,pTax,pTotRating,pNumRatings,pBackOrder,'.(@$manufacturerfield!=''?'mfName,':'')."'' AS " . getlangid('pDescription',2) . ',' . getlangid('pLongDescription',4) . ' FROM products '.(@$manufacturerfield!=''?'LEFT OUTER JOIN manufacturer on products.pManufacturer=manufacturer.mfID ':'').'WHERE pDisplay<>0 AND pId IN (' . $prodlist . ')';
