...
if($usecategoryformat==1 || $usecategoryformat==2) $numcolumns=2*$categorycolumns; else $numcolumns=$categorycolumns;
	if(! @$usecsslayout) print '<table width="100%" border="0" cellspacing="3" cellpadding="3">';
if($mess1!=''){
...
if($success){
	if(@$noshowdiscounts != TRUE){
		...
		if(mysql_num_rows($result2) > 0){
			...
			print '<div class="discountsapply allcatdiscounts"' . (@$nomarkup ? '' : ' style="font-weight:bold;"') . '>' . $xxDsCat . '</div><div class="catdiscounts allcatdiscounts"' . (@$nomarkup ? '' : ' style="font-size:9px;color:#FF0000;"') . '>';
	...
	if(! (@isset($showcategories) && @$showcategories==FALSE)){
		if(@$usecsslayout) print '<div class="prodnavigation">'; else print '<table width="98%" border="0" cellspacing="3" cellpadding="3"><tr>';
		if(@$allproductsimage!='') print (@$usecsslayout ? '<div' : '<td width="5%" align="right"') . ' class="catimage"><a class="ectlink" href="products.php"><img class="catimage" src="' . @$allproductsimage . '" border="0" alt="' . $xxAlProd . '" /></a>' . $afterimage . (@$usecsslayout ? '</div>' : '</td>');
		...
		if($xxAlPrCa!='' && ! $manufacturers) print (@$usecsslayout ? '<div' : '<p') . ' class="navdesc">' . $xxAlPrCa . @$catseparator . (@$usecsslayout ? '</div>' : '</p>');
	...
	while($rs=mysql_fetch_row($result)){
		...
		if(($usecategoryformat==1 || $usecategoryformat==2) && ! $noimage){
			$cellwidth -= 5;
			print (@$usecsslayout ? '<div' : '<td width="5%" align="right"') . ' class="catimage">' . $startlink . '<img alt="' . str_replace('"','',$rs[1]) . '" class="catimage" src="' . $rs[3] . '" border="0" /></a>' . $afterimage . (@$usecsslayout ? '</div>' : '</td>');
		}
		...
		if(@$nocategoryname!=TRUE) print (@$usecsslayout ? '<div class="catname">' : '<p class="catname"><strong>') . $startlink . $rs[1] . '</a>' . $xxDot . (! @$usecsslayout ? '</strong>' : '');
		if($alldiscounts!='') print ' <span class="eachcatdiscountsapply eachcatdiscount"' . (@$nomarkup ? '' : ' style="color:#FF0000;font-weight:bold;"') . '>' . $xxDsApp . '</span>' . (@$nomarkup?'':'<font size="1">') . '<div class="catdiscounts eachcatdiscount">' . $alldiscounts . '</div>' . (@$nomarkup?'':'</font>');
