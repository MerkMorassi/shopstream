...
if(! (@isset($showcategories) && @$showcategories==FALSE)){
	...
	print '<' . (@$usecsslayout ? 'div' : 'td align="right"') . ' class="checkoutbutton">&nbsp;' . (@$nobuyorcheckout!=TRUE ? imageorbutton($imgcheckoutbutton,$xxCOTxt,'checkoutbutton','cart.php', FALSE) : '') . '</' . (@$usecsslayout ? 'div' : 'td') . '>' . "\r\n";
...
if(@$noshowdiscounts != TRUE){
	...
	if(mysql_num_rows($allprods) == 0)
		...
		print $sstrong . '<div class="'.$cs.'prodname">' . $startlink . $rs[getlangid("pName",1)] . $endlink . $xxDot . '</div>' . $estrong;
		...
		if($thedesc!='') print '<div class="'.$cs.'proddescription">' . $thedesc . '</div>'; else print '<br />';
