...
?>
		...
		<?php	if(@$_POST['act']=="modify" && is_numeric(@$_POST['id'])){ ?>
...
<?php
	function displayheaderrow(){
		global $yyWrkNam,$yyType,$yyExpDat,$yyGlobal,$yyClone,$yyModify,$yyDelete; ?>
			  <tr>
...
<?php
	...
	$tmpSQL = 'SELECT COUNT(*) AS bar FROM coupons';
	$allprods = mysql_query($tmpSQL) or print(mysql_error());
	$iNumOfPages = ceil(mysql_result($allprods,0,'bar')/$admindiscountsperpage);
	mysql_free_result($allprods);
	$sSQL = "SELECT cpnID,cpnWorkingName,cpnSitewide,cpnIsCoupon,cpnEndDate FROM coupons ORDER BY cpnIsCoupon,cpnWorkingName LIMIT " . ($admindiscountsperpage*($CurPage-1)) . ', ' . $admindiscountsperpage;
	$result = mysql_query($sSQL) or print(mysql_error());
	if(mysql_num_rows($result) > 0){
		$pblink = '<a href="admindiscounts.php?pg=';
		if($iNumOfPages > 1) print '<tr><td colspan="7" align="center">' . writepagebar($CurPage,$iNumOfPages,$yyPrev,$yyNext,$pblink,FALSE) . '<br />&nbsp;</td></tr>';
		displayheaderrow();
...
<?php	}
		if($iNumOfPages > 1) print '<tr><td colspan="7" align="center"><br />' . writepagebar($CurPage,$iNumOfPages,$yyPrev,$yyNext,$pblink,FALSE) . '</td></tr>';
	}else{
?>
			  <tr> 
