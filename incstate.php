...
if($adminAltRates>0){
	...
	$alternateratesweightbased=(mysql_num_rows($result)>0);
	...
	foreach(@$_POST as $objItem => $objValue){
		if(substr($objItem,0,9)=='stateName'){
			...
			if($stateName!=''){
				...
				$gotstateid=FALSE;
				while(! $gotstateid){
					$result = mysql_query("SELECT stateID FROM states WHERE stateID=" . $nextfreeid) or print(mysql_error());
					if(mysql_num_rows($result)==0) $gotstateid=TRUE; else $nextfreeid++;
					mysql_free_result($result);
				}
				$sSQL = "INSERT INTO states (stateID,stateName,stateName2,stateName3,stateCountryID,stateEnabled) VALUES (" .
					...
					"'" . escape_string($stateName3) . "', " . @$_POST['thiscountry'] . ',1)';
				mysql_query($sSQL);
			}
		}
	}
	print '<meta http-equiv="refresh" content="1; url=adminstate.php?thiscountry=' . @$_POST['thiscountry'] . '" />';
...
if(@$_POST['doeditstates']!=''){ ?>
			<p align="center"><br />&nbsp;<br />&nbsp;<br /><strong><?php print $yyUpdSuc?></strong><br /><br /><?php print $yyNowFrd?><br /><br />
                        ...
                        <br /><br /><br /></p>
<?php
}elseif((@$_POST['posted']=='1' || @$_POST['setallact']!='') && ! $success){ ?>
...
<?php		}
				  ...
				  <table width="340" border="0" cellspacing="1" cellpadding="3" class="cobtbl">
					...
					<td class="cobll" align="left"><select size="1" name="thiscountry" id="thiscountry" onchange="document.location='adminstate.php?thiscountry='+this[this.selectedIndex].value"><?php
						...
						mysql_free_result($result);
						if(is_numeric(@$_GET['loadstates']))
							mysql_query("UPDATE countries SET loadStates=" . $_GET['loadstates'] . " WHERE countryID=" . $thiscountry) or print(mysql_error());
						$sSQL = "SELECT loadStates FROM countries WHERE countryID=" . $thiscountry;
						$result = mysql_query($sSQL) or print(mysql_error());
						if($rs=mysql_fetch_assoc($result)) $loadstates=$rs['loadStates']; else $loadstates=0;
						mysql_free_result($result);
					?></select></td></tr>
...
<?php
	if(@$_GET['editstates']=='1' && is_numeric($_GET['thiscountry'])){ ?>
		  <input type="hidden" name="doeditstates" value="1" />
		  <input type="hidden" name="thiscountry" value="<?php print $_GET['thiscountry']?>" />
<?php	$sSQL="SELECT countryName FROM countries WHERE countryID=" . @$_GET['thiscountry'];
		$result = mysql_query($sSQL) or print(mysql_error());
		if($rs=mysql_fetch_assoc($result)) $countryName=$rs['countryName']; else $countryName="UNDEFINED";
		mysql_free_result($result);
?>
...
<?php	$sSQL = "SELECT stateID,stateName,stateName2,stateName3,stateEnabled FROM states WHERE stateCountryID=" . $thiscountry . " ORDER BY stateName";
		$result = mysql_query($sSQL) or print(mysql_error());
		$hasrows=(mysql_num_rows($result)>0);
		while($rs=mysql_fetch_assoc($result)){
...
<?php	if($editzones) print '<td class="cobhl" align="center"><strong>' . $yyPZone . '</strong></td>';
		if($thiscountry==$origCountryID || $forcezonesplit) print '<td class="cobhl" align="center"><strong>' . $yyModify . '</strong></td>'; ?>
			  </tr><?php
		$theids = @$_POST['ids'];
		$bgcolor='cobhl';
