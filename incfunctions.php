...
if(@$usecsslayout){ $nomarkup=TRUE; $useproductbodyformat=2; $usesearchbodyformat=2; $usedetailbodyformat=4; }
if(@$giftcertificateid=='') $giftcertificateid='giftcertificate';
if(@$donationid=='') $donationid='donation';
if(@$giftwrappingid=='') $giftwrappingid='giftwrapping';
if(@$mobilebrowser!=TRUE) $mobilebrowser=detectmobilebrowser();
if($mobilebrowser){ $inlinecheckout=TRUE; $hideshipaddress=TRUE; if(@$mobilebrowsercolumns!='') $productcolumns=$mobilebrowsercolumns; }
if(@$adminencoding=='') $adminencoding='iso-8859-1';
if(@$emailencoding=='') $emailencoding=$adminencoding;
...
function getadminsettings(){
	...
	if(! @$alreadygotadmin){
		...
		$adminShipping=$shipType=(int)$rs['adminShipping'];
	...
	if((substr(strtolower($storeurl),0,7) != 'http://') && (substr(strtolower($storeurl),0,8) != 'https://') && $storeurl!='')
		$storeurl = 'http://' . $storeurl;
	if(substr($storeurl,-1)!='/' && $storeurl!='') $storeurl .= '/';
	if(@$oremailaddr!='') $emailAddr=$oremailaddr;
	// Language
	global $ssIncTax,$xxStaTax,$xxCntTax,$xxPostco;
	if($origCountryCode=="GB" || $origCountryCode=="IE"){
		$ssIncTax=str_replace("Tax","VAT",$ssIncTax);
		$xxCntTax="VAT";
	}elseif($origCountryCode=="AU" || $origCountryCode=="CA"){
		$xxStaTax="PST";
		$xxCntTax="GST";
	}
	if($origCountryCode=="CA" && $storelang==''){
...
function efformvalidator(theForm){
	if(document.getElementById('yourname').value==""){
		alert("<?php print jscheck($xxPlsEntr . ' "' . $xxEFNam)?>\".");
		document.getElementById('yourname').focus();
		return(false);
	}
	if(document.getElementById('youremail').value==""){
		alert("<?php print jscheck($xxPlsEntr . ' "' . $xxEFEm)?>\".");
		document.getElementById('youremail').focus();
		return(false);
	}
	if(document.getElementById('askq').value!='1'){
		if(document.getElementById('friendsemail').value==""){
			alert("<?php print jscheck($xxPlsEntr . ' "' . $xxEFFEm)?>\".");
...
global $prodoptions,$countryTaxRate,$xxPrdEnt,$xxPrdChs,$xxPrd255,$xxOptOOS,$xxOpSkTx,$xxInStNo,$xxValEm,$xxNoStEm,$xxEmail,$xxSubmt,$xxCancel,$useStockManagement,$prodlist,$OWSP,$xxEntMul,$noupdateprice,$noprice,$hideoptpricediffs,$showinstock,$noshowoptionsinstock,$showtaxinclusive,$notifybackinstock,$xxInvCha,$xxBakOpt,$absoptionpricediffs;
global $currSymbol1,$currFormat1,$currSymbol2,$currFormat2,$currSymbol3,$currFormat3,$pricecheckerisincluded,$xxDigits,$sstrong,$estrong,$currRate1,$currSymbol1,$currRate2,$currSymbol2,$currRate3,$currSymbol3,$currencyseparator,$enablewishlists,$wishlistonproducts,$xxMyWisL,$pricezeromessage,$xxMaxChr,$inlinepopups,$txtcollen;
...
<tr><td class="cobll" bgcolor="#FFFFFF" colspan="2"><?php print $xxNoStEm?></td></tr>
...
<tr><td class="cobll" bgcolor="#FFFFFF" colspan="2" align="center"><input type="button" value="<?php print $xxSubmt?>" onclick="regnotifystock()" /> &nbsp; <input type="button" value="<?php print $xxCancel?>" onclick="closeinstock()" /></td></tr>
</table></div>
<?php	}
?><input type="hidden" id="hiddencurr" value="<?php print str_replace('"','&quot;',FormatEuroCurrency(0))?>" /><script language="javascript" type="text/javascript">
/* <![CDATA[ */
...
function notifystockcallback(){
	if(nsajaxobj.readyState==4){
		...
		if(rstxt!='SUCCESS')alert(rstxt);else alert("<?php print jscheck($xxInStNo)?>");
		closeinstock();
	}
}
...
if(backorder&&(globBakOrdChk||confirm("<?php print jscheck($xxBakOpt)?>")))return(globBakOrdChk=true);
<?php	if(@$notifybackinstock){ ?>
notifyinstock(true,x.form.id.value,x.form.id.value,i);
<?php	}else{ ?>
alert("<?php print jscheck($xxOptOOS)?>");
<?php	} ?>
...
alert("<?php print jscheck($xxPrdEnt)?>");
x.focus();return(false);}
function invalidChars(x){
alert("<?php print jscheck($xxInvCha)?>" + x);
return(false);}
function enterDigits(x){alert("<?php print jscheck($xxDigits)?>");x.focus();return(false);}
function enterMultValue(){alert("<?php print jscheck($xxEntMul)?>");return(false);}
function chooseOption(x){
alert("<?php print jscheck($xxPrdChs)?>");
x.focus();return(false);}
function dataLimit(x,numchars){
alert("<?php print jscheck($xxPrd255)?>".replace(255,numchars));
x.focus();return(false);}
var hiddencurr='';
...
<?php
	if(@$noprice!=TRUE && @$hideoptpricediffs!=TRUE) print "if(theoptpricediff!=0)optns.options[i].text+=' ('+".(@$absoptionpricediffs?'':"(theoptpricediff>0?'+':'-')+").'formatprice(Math.abs(Math.round(('.(@$absoptionpricediffs?'prodprice+prodtax+':'').'theoptpricediff'.(@$showtaxinclusive===2?'+(!taxexmpt?theoptpricediff*thetax/100.0:0)':'').")*100)/100.0), '', '')+')';";
	...
	if($useStockManagement){ ?>
				...
				theoptpricediff=(theoptprice+staticpricediff)-totalprice;
<?php
	if(@$noprice!=TRUE && @$hideoptpricediffs!=TRUE) print "if(theoptpricediff!=0)optn.innerHTML+=' ('+".(@$absoptionpricediffs?'':"(theoptpricediff>0?'+':'-')+").'formatprice(Math.abs(Math.round(('.(@$absoptionpricediffs?'prodprice+prodtax+':'').'theoptpricediff'.(@$showtaxinclusive===2?'+(!taxexmpt?theoptpricediff*thetax/100.0:0)':'').")*100)/100.0), '', '')+')';";
	...
	if($useStockManagement){ ?>
				...
				theoptpricediff=(theoptprice+staticpricediff)-totalprice;
<?php
	if(@$noprice!=TRUE && @$hideoptpricediffs!=TRUE) print "if(theoptpricediff!=0)optnt.innerHTML+=' ('+".(@$absoptionpricediffs?'':"(theoptpricediff>0?'+':'-')+").'formatprice(Math.abs(Math.round(('.(@$absoptionpricediffs?'prodprice+prodtax+':'').'theoptpricediff'.(@$showtaxinclusive===2?'+(!taxexmpt?theoptpricediff*thetax/100.0:0)':'').")*100)/100.0), '', '')+')';";
...
if(backorder&&oneoutofstock&&!globBakOrdChk){if(!confirm("<?php print jscheck($xxBakOpt)?>"))return(false);}
<?php		} ?>
if(oos[prodnum]&&oos[prodnum]!=''&&!backorder){<?php if(@$notifybackinstock) print 'notifyinstock(true,otid[prodnum],rid[prodnum],0);'; else print 'alert("'.jscheck($xxOptOOS).'");'?>document.getElementById(oos[prodnum]).focus();return(false);}
return (true);
}
...
function setvals<?php print $Count?>(){<?php
		foreach($prodoptions as $rowcounter => $theopt){
			print 'optacpc[' . $rowcounter . "]='" . jsescape($theopt['optAcceptChars']) . "';optmaxc[" . $rowcounter . ']=' . $theopt['optTxtMaxLen'] . ';opttype['.$rowcounter.']=' . (int)$theopt['optType'] . ';optperc['.$rowcounter.']=' . (($theopt['optFlags'] & 1)==1 ? 'true' : 'false') . ";\r\n";
		} ?>
}
<?php
	} ?>
...
<?php
	if($doaddprodoptions && $prodoptions!=''){
		print 'setvals'.$Count.'();';
		print 'return(ectvalidate(theForm,'.count($prodoptions).','.$Count.','.($useStockManagement && $rs['pStockByOpts']!=0 ? 'true' : 'false').','.($rs['pBackOrder']!=0 ? 'true' : 'false').'));';
...
function updateprice<?php print $Count?>(){<?php
		print 'setvals'.$Count.'();';
		print 'updateprice('.count($prodoptions).','.$Count.','.$rs['pPrice'].",'".$rs['pId']."',".$thetax.','.($useStockManagement && $rs['pStockByOpts']!=0 ? 'true' : 'false').','.(($rs['pExemptions'] & 2)==2 ? 'true' : 'false').','.($rs['pBackOrder']!=0 ? 'true' : 'false').');'; ?>
}
<?php
	}
	...
	foreach($prodoptions as $rowcounter => $theopt){
		...
		if($rs2=mysql_fetch_array($result)){
			if(abs((int)$rs2['optType'])==3){
				...
				$optionshtml .= (@$usecsslayout ? '<div' : '<tr><td align="' . $tright . '" width="30%"') . ' class="optiontext' . ($isdetail ? ' detailoptiontext' : '') . '">' . $grpnmstyle . '<label for="optn'.$Count.'x'.$rowcounter.'">' . $rs2[getlangid('optGrpName',16)] . '</label>:' . $grpnmstyleend . (@$usecsslayout ? '</div><div' : '</td><td align="' . $tleft . '"') . ' class="option' . ($isdetail ? ' detailoption' : '') . '"> <input type="hidden" name="optn' . $rowcounter . '" value="' . $rs2['optID'] . '" />';
				if($fieldHeight != 1){
					$optionshtml .= '<textarea class="prodoption'.($isdetail?' detailprodoption':'').'" name="voptn' . $rowcounter . '" id="optn'.$Count.'x'.$rowcounter.'" cols="' . (int)$rs2["optDims"] . '" rows="' . $fieldHeight . '">';
					$optionshtml .= $rs2[getlangid('optName',32)] . '</textarea>';
				}else
					$optionshtml .= '<input type="text" class="prodoption'.($isdetail?' detailprodoption':'').'" maxlength="255" name="voptn' . $rowcounter . '" id="optn'.$Count.'x'.$rowcounter.'" size="' . $rs2['optDims'] . '" value="' . htmldisplay($rs2[getlangid('optName',32)]) . '" alt="' . $rs2[getlangid('optGrpName',16)] . '" />';
				$optionshtml .= (@$usecsslayout ? '</div>' : '</td></tr>');
			}elseif(abs((int)$rs2['optType'])==1){
				$optionshtml .= (@$usecsslayout ? '<div' : '<tr><td align="' . $tright . '" valign="baseline" width="30%"') . ' class="optiontext' . ($isdetail ? ' detailoptiontext' : '') . '">' . $grpnmstyle . $rs2[getlangid('optGrpName',16)] . ':' . $grpnmstyleend . (@$usecsslayout ? '</div><div' : '</td><td align="' . $tleft . '"') . ' class="option' . ($isdetail ? ' detailoption' : '') . '"> ';
				$index=0;
				do {
					...
					$optionshtml .= '<input type="'.(mysql_num_rows($result)==1?'checkbox':'radio').'" class="prodoption'.($isdetail?' detailprodoption':'').'" style="vertical-align:middle" onclick="updateoptimage('.$Count.','.$rowcounter.',1);updateprice'.$Count.'();" name="optn'.$Count.'x'.$rowcounter.'" ';
				...
				if((int)$rs2['optGrpSelect']!=0 && ! $isdetail){
					...
					$optionshtml.=(@$usecsslayout ? '<div' : '<tr><td align="center" colspan="2">&nbsp;<br /><table') . ' class="multioptiontable">';
					$index = 0;
					do {
						...
						if(($index % $multipurchasecolumns) == 0 && ! @$usecsslayout) $optionshtml .= '<tr>';
						$optionshtml .= (@$usecsslayout ? '<div' : '<td width="'.$colwid.'%" align="' . $tleft . '" style="white-space:nowrap"') . ' class="optiontext' . ($isdetail ? ' detailoptiontext' : '') . ' multioptiontext' . ($isdetail ? ' detailmultioptiontext' : '') . '">';
						...
						$optionshtml .= '</span></label>' . (@$usecsslayout ? '</div>' : '</td>');
						$index++;
						if(($index % $multipurchasecolumns) == 0 && ! @$usecsslayout) $optionshtml .= '</tr>';
						addtooptionsjs($optionsjs, $isdetail, $origoptpricediff);
					} while($rs2=mysql_fetch_assoc($result));
					...
					$hasmulti = 1;
					$optionshtml.=(@$usecsslayout ? '</div>' : '</table></td></tr>');
				}
			}else{
				$optionshtml .= (@$usecsslayout ? '' : '<tr>') . (@$noselectoptionlabel!=TRUE ? '<' . (@$usecsslayout ? 'div' : 'td align="' . $tright . '" width="30%"') . ' class="optiontext' . ($isdetail ? ' detailoptiontext' : '') . '">' . $grpnmstyle . '<label for="optn'.$Count.'x'.$rowcounter.'">' . $rs2[getlangid('optGrpName',16)] . '</label>:' . $grpnmstyleend . (@$usecsslayout ? '</div><div' : '</td><td align="' . $tleft . '"') . ' class="option' . ($isdetail ? ' detailoption' : '') . '"> ' : (@$usecsslayout ? '<div' : '<td colspan="2"') . ' class="prodoption selectoption'.($isdetail?' detailprodoption':'').'">') . '<select class="prodoption'.($isdetail?' detailprodoption':'').'" onchange="updateprice'.$Count.'();updateoptimage('.$Count.','.$rowcounter.');" name="optn' . $rowcounter . '" id="optn'.$Count.'x'.$rowcounter.'" size="1">';
				...
				$optionshtml .= '</select>' . (@$usecsslayout ? '</div>' : '</td></tr>');
			}
		}
		mysql_free_result($result);
...
function rethuni($tnum){
	return('\\u'.str_pad(dechex($tnum),4,'0',STR_PAD_LEFT));
}
function jscheck($thetxt){
	return str_replace('"','\\"',preg_replace('/&#(\d+);/me',"rethuni(\\1)",$thetxt));
}
...
function dosendemail($doseto, $dosefrom, $dosereplyto, $dosesubject, $dosebody){
	global $doencodeemailsubject,$customheaders,$emailencoding,$htmlemails,$debugmode,$usemailer,$smtphost,$smtpusername,$smtppassword,$smtpport,$smtpsecure,$emailflags,$emailcr;
...
function detectmobilebrowser(){
	...
	$dmb=FALSE;
	if(strpos($uagent,'android')!==FALSE) $dmb=TRUE;
	if(strpos($uagent,'blackberry')!==FALSE) $dmb=TRUE;
	if(strpos($uagent,'iphone')!==FALSE) $dmb=TRUE;
	if(strpos($uagent,'mobile')!==FALSE) $dmb=TRUE;
	if(strpos($uagent,'nokia')!==FALSE) $dmb=TRUE;
	if(strpos($uagent,'opera mini')!==FALSE) $dmb=TRUE;
	if(strpos($uagent,'pocketpc')!==FALSE) $dmb=TRUE;
	if(strpos($uagent,'samsung')!==FALSE) $dmb=TRUE;
	if(strpos($uagent,'symbian')!==FALSE) $dmb=TRUE;
	if(strpos($uagent,'smartphone')!==FALSE) $dmb=TRUE;
	return($dmb);
}
function updaterchecker(){
	global $yyNoNew,$yyLasChk,$yyChkMan,$yyClkHer,$yyNewRec,$yyRUSec,$yyChkNew,$disableupdatechecker,$nopadsscompliance,$yyWilLog,$yyCliCon,$yyFinMor,$yyClkHer,$yyContin,$yyCancel;
