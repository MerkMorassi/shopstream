...
function ParseDHLXMLOutput($sXML, $international, &$errormsg, &$errorcode, &$intShipping){
	global $xxDays,$numuspsmeths,$uspsmethods;
	$noError=TRUE;
						...
						for($k = 0; $k < $nodeList3->length; $k++){
							if($nodeList3->nodeName[$k]=='QtdShp'){
								...
								if($shippingcharge>0){
									...
									$intShipping[$l][1] = $nodeList4->getValueByTagName('TotalTransitDays') . ' ' . $xxDays;
									...
									if(! $wantthismethod) $intShipping[$l][3]=0; else $intShipping[$l][3]=TRUE;
								}
							}
						}
					}
...
function ParseUSPSXMLOutput($sXML, $international,&$errormsg,&$intShipping){
	global $xxDay,$xxDays,$dumpshippingxml,$numuspsmeths,$uspsmethods;
	$noError = TRUE;
	$packCost = 0;
	$errormsg = '';
	$xmlDoc = new vrXMLDoc($sXML);
...
function ParseNewCanadaPostXMLOutput($sXML, $international,&$errormsg,&$errorcode,&$intShipping){
	global $xxDay,$xxDays,$destZip,$storelang;
	$noError = TRUE;
	$errormsg = '';
	$discntsApp = '';
	$l = 0;
		...
		if($nodeList->nodeName[$i]=='soap:Fault'){
			...
			for($j = 0; $j < $e->length; $j++){
				if($e->nodeName[$j]=='faultcode'){
					...
					if(strpos($errormsg,'}PostalCodeType')!==FALSE){
			...
			for($j = 0; $j < $e->length; $j++){
				if($e->nodeName[$j] == 'price-quote'){
					...
					for($k = 0; $k < $t->length; $k++){
						...
						}elseif($t->nodeName[$k]=='service-standard'){
							$ep = $t->childNodes[$k];
							for($pj = 0; $pj < $ep->length; $pj++){
...
function getuspscontainer($gpcweight,$theservice){
	...
	if($ispriority && $gpcweight<=20 && ($packdims[0]<=8.625 && $packdims[1]<=5.375 && $packdims[2]<=1.625)) $getuspscont='sm flat rate box';
	if($isexpress && $gpcweight<=20 && (($packdims[0]<=11 && $packdims[1]<=8.5 && $packdims[2]<=13.625) || ($packdims[0]<=5.5 && $packdims[1]<=11.875 && $packdims[2]<=3.375))) $getuspscont='flat rate box';
	...
	for($index=0;$index<$numuspsmeths;$index++){
		...
		if($uspsmethods[$index][0]=='PRIORITY'){
			if(($adminUnits & 12)!=0){
				...
				if(@$uspsprioritycontainer=='auto') $uspsprioritycontainer=getuspscontainer($iWeight,'PRIORITY');
			}
		...
		if($uspsmethods[$index][0]=='EXPRESS'){
			if(($adminUnits & 12)!=0 && @$uspsexpresscontainer=='auto') $uspsexpresscontainer=getuspscontainer($iWeight,'EXPRESS');
...
function addUSPSInternational($id,$iWeight,$quantity,$mailtype,$country,$packcost){
	global $packdims,$numuspsmeths,$uspsmethods,$adminUnits,$shipCountryCode,$gxgtoobig,$iTotItems;
	$iTotItems++;
	if(($adminUnits & 12)!=0){
		...
		for($xx=0; $xx < $numuspsmeths; $xx++){
			...
			if($uspsmethods[$xx][0]=='24') if(! doesfitinbox(7.5625,5.4375,0.625)) $uspsmethods[$xx][0]=='xxx'; // DVD FRB
			if($uspsmethods[$xx][0]=='16') if(! doesfitinbox(8.625, 5.375, 1.625)) $uspsmethods[$xx][0]=='xxx'; // Small FRB
			if($uspsmethods[$xx][0]=='20') if(! doesfitinbox(10,6,0.75)) $uspsmethods[$xx][0]=='xxx'; // Small FRE
			if($uspsmethods[$xx][0]=='9' || $uspsmethods[$xx][0]=='26') if(! doesfitinbox(11,8.5,5.5) && ! doesfitinbox(13.625,11.875,3.375)) $uspsmethods[$xx][0]=='xxx'; // FRB
			if($uspsmethods[$xx][0]=='13') if(! doesfitinbox(11.5,6.125,0.25)) $uspsmethods[$xx][0]=='xxx'; // FirstClass Letter
			if($uspsmethods[$xx][0]=='11') if(! doesfitinbox(12,12,5.5)) $uspsmethods[$xx][0]=='xxx'; // LFRB
			if($uspsmethods[$xx][0]=='8' || $uspsmethods[$xx][0]=='10') if(! doesfitinbox(12.5,9.5,1)) $uspsmethods[$xx][0]=='xxx'; // FRE
			if($uspsmethods[$xx][0]=='17') if(! doesfitinbox(15,9.5,0.75)) $uspsmethods[$xx][0]=='xxx'; // Legal FRE
			if($uspsmethods[$xx][0]=='14') if(! doesfitinbox(15,12,0.75)) $uspsmethods[$xx][0]=='xxx'; // FirstClass L-E
			
			if($packdims[0]>7.5625 || $packdims[1]>5.4375 || $packdims[2]>0.625){
				if($uspsmethods[$xx][0]=='24') $uspsmethods[$xx][0]='xxx'; // DVD FRB
			}
			if($packdims[0]>8.625 || $packdims[1]>5.375 || $packdims[2]>1.625){
				if($uspsmethods[$xx][0]=='16') $uspsmethods[$xx][0]='xxx'; // Small FRB
			}
			if($packdims[0]>10 || $packdims[1]>6 || $packdims[2]>0.75){
				if($uspsmethods[$xx][0]=='20') $uspsmethods[$xx][0]='xxx'; // Small FRE
			}
			...
			if($packdims[0]>12 || $packdims[1]>12 || $packdims[2]>5.5){
			...
			if($packdims[0]>15 || $packdims[1]>9.5 || $packdims[2]>0.75){
				if($uspsmethods[$xx][0]=='17') $uspsmethods[$xx][0]='xxx'; // Legal FRE
			}
	...
	if(($adminUnits & 12)!=0 && ceil($packdims[0])>0 && ceil($packdims[1])>0 && ceil($packdims[2])>0) $sXML .= '<Width>' . round($packdims[2],2) . '</Width><Length>' . round($packdims[0],2) . '</Length><Height>' . round($packdims[1],2) . '</Height><Girth>' . ceil(($packdims[1]*2)+($packdims[2]*2)) . '</Girth>'; else $sXML .= '<Width></Width><Length></Length><Height></Height><Girth></Girth>';
...
function addDHLPackage($iWeight,$adminUnits,$packTypeCode,$country,$packcost,$dimens){
	...
	$tempXML .= '<Weight>' . $iWeight . '</Weight></Piece>';
	$packnumber++;
	return($tempXML);
}
...
function addCanadaPostPackage($iWeight,$adminUnits,$packTypeCode,$country,$packcost,&$dimens){
	global $addshippinginsurance,$packtogether,$canadapostusername,$adminUnits;
	if($iWeight<0.1) $iWeight=0.1;
...
function CanadaPostCalculate($sXML,$international,&$errormsg,&$intShipping){
	global $pathtocurl,$usecurlforfsock,$curlproxy,$destZip,$xxPlsZip,$dumpshippingxml,$shipCountryID,$canadapostusername;
	$success = true;
	if($destZip=='' && ! zipisoptional($shipCountryID)){
		$errormsg=$xxPlsZip;
		return(FALSE);
	...
	if(@$canadapostusername!=''){
		$cpurl='https://' . (FALSE?'ct.':'') . 'soa-gw.canadapost.ca/rs/soap/rating';
		$success = callcurlfunction($cpurl, $sXML, $res, '', $errormsg, FALSE);
	}elseif(@$usecurlforfsock){
	...
	if($success){
		if(@$canadapostusername!='')
			$success = ParseNewCanadaPostXMLOutput($res, $international,$errormsg,$errorcode,$intShipping);
		else
